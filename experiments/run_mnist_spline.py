...
#
# Same pipeline as ``run_mnist_single_layer.py``, but activation profiles on the
# ``2**n_qubits`` grid (e.g. **32 points** for ``n_qubits=5``) are read with a
# **natural cubic spline** through those samples instead of piecewise-linear
# interpolation. Training still uses the superposition parameterization; only the
# map from discrete profile values to the continuous pre-activation coordinate is
# smoother.
#
...
OUTPUT_ROOT = PROJECT_ROOT / "notebooks" / "note10_spline_outputs" / RUN_ID
