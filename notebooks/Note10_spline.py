# %% [markdown]
# # MNIST With Smooth Spline Profile Readout (Note 10 variant)
#
# Same pipeline as ``Note10.py``, but activation profiles on the ``2**n_qubits`` grid (e.g. **32 points** for ``n_qubits=5``) are read with a **natural cubic spline** through those samples instead of piecewise-linear interpolation. Training still uses the superposition parameterization; only the map from discrete profile values to the continuous pre-activation coordinate is smoother.
#
# Artifacts go under ``notebooks/note10_spline_outputs/<run_id>/`` (separate from ``note10_outputs``).
#
# To try **KAN-style cubic B-splines** (coefficients on an open uniform knot vector, not interpolating the grid values), set ``profile_interp = "cubic_bspline"`` below (works with JAX training).
...
OUTPUT_ROOT = Path(__file__).resolve().parent / "note10_spline_outputs" / RUN_ID
...
def _note10_spline_restore_stdio() -> None:
...
atexit.register(_note10_spline_restore_stdio)
...
# ## Config
#
# ``profile_interp="cubic_natural"``: **C²** curve through the ``2**n_qubits`` profile samples on ``[-1,1]``. Alternatives: ``"linear"`` (original), ``"cubic_bspline"`` (B-spline coefficients; SciPy on NumPy predict path).
#
...
# %% [markdown]
# ## 3. Standard Superposition Activations (spline readout)
#

# %%
standard_result = run_quantum_experiment(
    "standard",
    label="MNIST standard superposition (cubic_natural profile readout)",
    split=mnist_split,
    hidden_units=hidden_units,
    n_qubits=n_qubits,
    steps=steps,
...
# %% [markdown]
# ## 4. Mode A (spline readout)
#

# %%
mode_a_result = run_quantum_experiment(
    "mode_a",
    label="MNIST Mode A (cubic_natural profile readout)",
    split=mnist_split,
    hidden_units=hidden_units,
    n_qubits=n_qubits,
    steps=steps,
...
# %% [markdown]
# ## 5. Mode B (spline readout)
#

# %%
mode_b_result = run_quantum_experiment(
    "mode_b",
    label="MNIST Mode B (cubic_natural profile readout)",
    split=mnist_split,
    hidden_units=hidden_units,
    n_qubits=n_qubits,
    steps=steps,
